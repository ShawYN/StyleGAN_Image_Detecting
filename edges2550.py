...
def edge(img):
    ...
    edge_output=cv.Canny(xgrad,ygrad,25,50)
...
for filename in os.listdir('J:\\Research_lab_data_base\\train\\'): 
    ...
    if(str(filename).split(".")[-3] == "fake"):
        cv.imwrite("J:\\Research_lab_data_base\\train\\"+"fake.2550"+str(i).zfill(6)+".png",edge_output)
    else:
        cv.imwrite("J:\\Research_lab_data_base\\train\\"+"real.2550"+str(i).zfill(6)+".png",edge_output)
